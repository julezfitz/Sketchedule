...
VisuSchedule is a mobile app for making and storing quick personalised visual schedules. VisuSchedule allows users to to create schedules by either uploading personal photographs, or by selecting from a searchable database of photos. VisuSchedule has both schedule edit and view modes, so once a schedule is ready, view mode switches to a linear scroll from start to finish, allowing items to be checked off as they go. Once all items are complete, clicking the complete button resets all items status for use again another time.
